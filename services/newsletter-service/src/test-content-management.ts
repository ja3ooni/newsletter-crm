...
async function testContentManagementSystem() {
  const logger = console; // Keep console for test files as they are not production code
  ...
  try {
    ...
    const performanceReport =
      await contentAnalyticsService.getContentPerformanceReport(
        'content-123',
        30
      );
