...
export class EventService {
  ...
  async createEvent(
    ...
    try {
      const event = await this.eventRepository.createEvent(type, contactId, data, source);
      ...
      logger.error('Error creating event', { error, type, contactId, data, source });
      throw error;
    }
  }
  ...
  async processEvent(eventId: string): Promise<void> {
    try {
      ...
      const triggers = await this.eventRepository.findEventTriggersByType(event.type);
  ...
  async processUnprocessedEvents(batchSize: number = 100): Promise<number> {
    try {
      const events = await this.eventRepository.findUnprocessedEvents(batchSize);

      if (events.length === 0) {
        return 0;
      }

      logger.info('Processing batch of unprocessed events', { count: events.length });
  ...
  private async processTrigger(trigger: EventTrigger, event: AutomationEvent): Promise<void> {
    // Evaluate trigger conditions
    const conditionsMet = await this.evaluateTriggerConditions(trigger.conditions, event);
  ...
  private async evaluateTriggerConditions(
    ...
    for (const condition of conditions) {
      const conditionResult = this.evaluateSingleTriggerCondition(condition, event);
  ...
  private async triggerWorkflow(workflowId: string, event: AutomationEvent): Promise<void> {
    try {
      const workflow = await this.workflowRepository.findById(workflowId);

      if (!workflow) {
        logger.error('Workflow not found for trigger', { workflowId, eventId: event.id });
        return;
      }
  ...
  private async triggerDripCampaign(campaignId: string, event: AutomationEvent): Promise<void> {
    try {
      const campaign = await this.dripCampaignRepository.findById(campaignId);

      if (!campaign) {
        logger.error('Drip campaign not found for trigger', { campaignId, eventId: event.id });
        return;
      }
  ...
  async createEventTrigger(
    ...
    try {
      if (!workflowId && !campaignId) {
        throw new Error('Event trigger must have either workflowId or campaignId');
      }
  ...
  async getEventTriggers(
    ...
    try {
      return await this.eventRepository.findAllEventTriggers(pagination, filters);
    } catch (error) {
      logger.error('Error getting event triggers', { error, pagination, filters });
      throw error;
    }
  }
  ...
  async updateEventTrigger(
    ...
    try {
      const trigger = await this.eventRepository.updateEventTrigger(id, updates);
  ...
  async activateEventTrigger(id: string): Promise<EventTrigger | null> {
    try {
      return await this.eventRepository.updateEventTrigger(id, { isActive: true });
    } catch (error) {
  ...
  async deactivateEventTrigger(id: string): Promise<EventTrigger | null> {
    try {
      return await this.eventRepository.updateEventTrigger(id, { isActive: false });
    } catch (error) {
  ...
  async getEventsByContact(
    ...
    try {
      return await this.eventRepository.findEventsByContact(contactId, pagination, filters);
    } catch (error) {
      logger.error('Error getting events by contact', { error, contactId, pagination, filters });
      throw error;
    }
  }
  ...
  async getEventsByType(
    ...
    try {
      return await this.eventRepository.findEventsByType(eventType, pagination, filters);
    } catch (error) {
      logger.error('Error getting events by type', { error, eventType, pagination, filters });
      throw error;
    }
  }
  ...
  async cleanupOldEvents(olderThanDays: number = 90): Promise<number> {
    try {
      const deletedCount = await this.eventRepository.deleteOldEvents(olderThanDays);
