...
router.post('/executions/:executionId/pause',
  [param('executionId').isUUID().withMessage('Invalid execution ID')],
...
// Workflow analytics
router.get('/workflows/:id/analytics',
  [param('id').isUUID().withMessage('Invalid workflow ID')],
...
router.delete('/drip-campaigns/:id',
  [param('id').isUUID().withMessage('Invalid campaign ID')],
...
router.post('/drip-campaigns/:id/pause',
  [param('id').isUUID().withMessage('Invalid campaign ID')],
  dripCampaignController.pauseDripCampaign.bind(dripCampaignController)
);

// Subscription management
router.post('/drip-campaigns/:id/subscribe',
  [
    param('id').isUUID().withMessage('Invalid campaign ID'),
    body('contactId').notEmpty().withMessage('Contact ID is required'),
    body('metadata').optional().isObject()
...
router.post('/subscriptions/:subscriptionId/unsubscribe',
...
router.get('/drip-campaigns/:id/subscriptions',
  [
    param('id').isUUID().withMessage('Invalid campaign ID'),
    query('page').optional().isInt({ min: 1 }),
    query('limit').optional().isInt({ min: 1, max: 100 })
...
router.get('/drip-campaigns/:id/analytics',
  [param('id').isUUID().withMessage('Invalid campaign ID')],
  dripCampaignController.getCampaignAnalytics.bind(dripCampaignController)
);

// Batch operations
router.post('/drip-campaigns/process-active',
...
router.get('/contacts/:contactId/events',
  [
...
router.get('/events/type/:eventType',
  [
...
router.post('/events/process-unprocessed',
